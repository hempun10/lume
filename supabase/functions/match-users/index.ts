 ...
 */
function isPastFallbackThreshold(createdAt: string, thresholdMs = 30_000): boolean {
...
function pairUsers(entries: QueueEntry[]): MatchPair[] {
	...
	for (const modeEntries of Object.values(byMode)) {
		...
		modeEntries.sort((a, b) => new Date(a.created_at).getTime() - new Date(b.created_at).getTime());
		...
		for (let i = 0; i < modeEntries.length; i++) {
			...
			for (let j = i + 1; j < modeEntries.length; j++) {
				...
				const score = interestScore(modeEntries[i].interests, modeEntries[j].interests);
				...
				// Match if they share interests, or if either has waited past threshold
				if (score > 0 || eitherPastThreshold) {
	...
	try {
		...
		if (unnotified?.length) {
			for (const entry of unnotified) {
				...
				const ok = await broadcastMatch(supabaseUrl, serviceRoleKey, entry.user_id, {
					room_id: entry.room_id,
					partner_id: entry.matched_with,
				});
				if (ok) {
					await supabase.from("match_queue").update({ notified: true }).eq("id", entry.id);
					retryCount++;
				}
			}
		}
		...
		for (const pair of pairs) {
			...
			if (roomError || !room) {
				console.error(`Failed to create room for ${pair.userA.user_id} + ${pair.userB.user_id}:`, roomError);
				continue;
			}
			...
			if (updateError) {
				console.error(`Failed to update queue for userA ${pair.userA.user_id}:`, updateError);
				continue;
			}
			...
			if (okA) {
				await supabase.from("match_queue").update({ notified: true }).eq("id", pair.userA.id);
			}
			if (okB) {
				await supabase.from("match_queue").update({ notified: true }).eq("id", pair.userB.id);
			}
		...
		const metrics = { queueDepth, pairsMade, expiredCount, retryCount, durationMs };
